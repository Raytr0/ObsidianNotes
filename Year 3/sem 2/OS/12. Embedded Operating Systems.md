...
### Boot loader 
• As with a general purpose OS, boot loader is responsible for loading the OS kernel into the memory. But since the BIOS-like functionality of a motherboard is not available, the developer needs to worry more about this step. • Functions of the boot loader are still the same: 
